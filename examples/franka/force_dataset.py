...
def main():
    ...
    for ep_idx in tqdm(range(source_dataset.num_episodes)):
        ...
        for i in range(to_idx - from_idx - 1):
            ...
            # --- 1. 处理 State (NumPy) ---
            # LeRobot Dataset 读取出来通常是 Torch Tensor，这里第一时间转为 Numpy
            ...
            # --- 2. Pi0 推理 (JAX) ---
            # OpenPi 的 image_tools 处理后通常返回 Numpy 或 JAX Array
            def prepare_img(img):
                # img 是 Tensor (C,H,W), 转 numpy -> (H,W,C)
            ...
            # [修正点] res["actions"] 是 Numpy 数组，直接切片，不要调 .cpu()
